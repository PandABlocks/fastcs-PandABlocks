...
class BitGroup:
    ...
    async def __call__(self, value: bool):
        await asyncio.gather(*[bit_attr.set(value) for bit_attr in self.bit_attributes])
...
class Blocks:
    def __init__(
        ...
        self._bits_group_names: list[tuple[AttrRW, list[PandaName]]] = []
        self._bit_group_attributes: list[tuple[AttrRW, list[AttrRW]]] = []
    ...
    def setup_post_introspection(self):
        self._link_bits_groups()

    def _link_bits_groups(self):
        for capture_attribute, bit_panda_names in self._bits_group_names:
            bit_attributes: list[AttrRW] = [
                self.get_attribute(panda_name) for panda_name in bit_panda_names
            ]  # type: ignore

            capture_attribute.set_update_callback(
                BitGroup(capture_attribute, bit_attributes)
            )
    ...
    def _make_ext_out_bits(
        ...
        bits_capture_attribute = self._make_ext_out(
            parent_block, panda_name, field_info, initial_values
        )

        self._bits_group_names.append(
            (
                bits_capture_attribute,
                [
                    PandaName.from_string(label)
    ...
    def _make_bit_mux(
        ...
        parent_block.add_attribute(
            ...
            AttrRW(
                ...
                handler=DefaultFieldHandler(delay_panda_name),
                group=WidgetGroup.INPUTS.value,
